...
// ─── Unit Types ───
// Each unit type has a label and a multiplier (how many pieces it represents)
const UNIT_TYPES = {
  piece:    { label: 'Piece',     multiplier: 1  },
  dozen:    { label: 'Dozen',     multiplier: 12 },
  halfpack: { label: 'Half Pack', multiplier: 6  },
  pack:     { label: 'Pack',      multiplier: 12 },
};

// Returns price multiplied by unit size
function unitPrice(basePrice, unitKey) {
  return basePrice * (UNIT_TYPES[unitKey]?.multiplier || 1);
}

// Returns the stock units consumed for a given qty of a unit type
...
function renderPosGrid(products) {
  ...
  const unitOptions = Object.entries(UNIT_TYPES).map(([key, u]) =>
    `<option value="${key}">${u.label}</option>`
  ).join('');
  ...
  grid.innerHTML = products.map(p => {
    ...
    return `<div class="product-grid-card ${isOut ? 'out' : ''}">
      ...
      <div class="pg-img" onclick="addToCart('${p.id}', document.getElementById('unit_${p.id}').value)">${imgHtml}</div>
      <h4>${escHtml(p.name)}</h4>
      <div class="pg-price">₦${fmt(p.sellingPrice)} / pc</div>
      <div class="pg-stock">${isOut ? 'Out of stock' : `${p.quantity} pcs left`}</div>
      <div class="pg-unit-wrap">
        <select class="pg-unit-select" id="unit_${p.id}" onchange="updateCardPrice('${p.id}', ${p.sellingPrice}, this.value)">
          ${unitOptions}
        </select>
...
function updateCardPrice(productId, basePrice, unitKey) {
  const product = posProducts.find(p => p.id === productId);
  if (!product) return;
  const card = document.querySelector(`#unit_${productId}`)?.closest('.product-grid-card');
  if (!card) return;
  const priceEl = card.querySelector('.pg-price');
  const mul = UNIT_TYPES[unitKey]?.multiplier || 1;
  const label = UNIT_TYPES[unitKey]?.label || 'Piece';
  if (priceEl) priceEl.textContent = `₦${fmt(basePrice * mul)} / ${label}`;
}
...
function addToCart(productId, unitKey) {
  ...
  const cartKey = `${productId}_${unitKey}`; // unique key per product+unit combo
  ...
  if (existing) {
    const addMore = stockNeeded;
    if (currentStockUsed + addMore > product.quantity) {
    ...
    const price = unitPrice(product.sellingPrice, unitKey);
    const costPrice = unitPrice(product.costPrice || 0, unitKey);
    cart.push({
      cartKey, id: productId,
      name: product.name,
      unitKey, unitLabel,
      price,        // price per selected unit
      costPrice,    // cost per selected unit
      qty: 1,
