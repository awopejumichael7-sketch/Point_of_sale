...
function openProductModal(productId = null) {
  ...
  // Reset form
  ['pName','pCategory','pID','pBarcode','pQty','pCost','pPrice','pSupplier'].forEach(id => {
...
async function saveProduct() {
  ...
  const data = {
    ...
    costPrice: cost, sellingPrice: price, supplier,
...
// ─── Export CSV ───
function exportCSV() {
  const headers = ['Name','Product ID','Barcode','Category','Quantity','Cost Price','Selling Price','Supplier','Date Added'];
  const rows = allProducts.map(p => [
    p.name, p.productId, p.barcode, p.category, p.quantity, p.costPrice, p.sellingPrice, p.supplier,
