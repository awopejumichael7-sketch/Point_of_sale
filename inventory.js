...
function openProductModal(productId = null) {
  ...
  // Reset form
  ['pName','pCategory','pID','pBarcode','pQty','pCost','pPrice','pPriceThreequarter','pPriceHalfpack','pPriceDozens','pPricePack','pSupplier'].forEach(id => {
...
async function saveProduct() {
  ...
  const data = {
    ...
    priceThreequarter, priceHalfpack, priceDozens, pricePack,
    supplier,
...
// ─── Export CSV ───
function exportCSV() {
  const headers = ['Name','Product ID','Barcode','Category','Quantity','Cost Price','Price (Piece)','Price (3/4)','Price (Half Pack)','Price (Dozen)','Price (Pack)','Supplier','Date Added'];
  const rows = allProducts.map(p => [
    p.name, p.productId, p.barcode, p.category, p.quantity,
    p.costPrice, p.sellingPrice,
    p.priceThreequarter || '', p.priceHalfpack || '', p.priceDozens || '', p.pricePack || '',
    p.supplier,
