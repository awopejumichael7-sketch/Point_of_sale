...
function renderProducts(products) {
  ...
  if (!products.length) {
    tbody.innerHTML = `<tr><td colspan="15" class="empty-row"><i class="fas fa-box-open"></i> No products found</td></tr>`;
    return;
  }
...
function openProductModal(productId = null) {
  ...
  // Reset form
  ['pName','pCategory','pID','pBarcode','pQty','pCost','pPrice','pPriceHalf','pPriceThreequarter','pPriceHalfpack','pPriceDozens','pPricePack','pSupplier'].forEach(id => {
...
async function saveProduct() {
  ...
  const data = {
    ...
    priceHalf, priceThreequarter, priceHalfpack, priceDozens, pricePack,
    supplier,
...
// ─── Export CSV ───
function exportCSV() {
  const headers = ['Name','Product ID','Barcode','Category','Quantity','Cost Price','Price (Piece)','Price (½)','Price (3/4)','Price (Half Pack)','Price (Dozen)','Price (Pack)','Supplier','Date Added'];
