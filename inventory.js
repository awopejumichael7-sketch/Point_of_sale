...
function renderProducts(products) {
  ...
  if (!products.length) {
    tbody.innerHTML = `<tr><td colspan="17" class="empty-row"><i class="fas fa-box-open"></i> No products found</td></tr>`;
    return;
  }
...
function openProductModal(productId = null) {
  ...
  // Reset form
  ['pName','pCategory','pID','pBarcode','pQty','pCost','pPrice','pPriceHalf','pPriceThreequarter','pPriceHalfpack','pPriceDozens','pPricePack','pPriceKilo','pPriceYard','pSupplier'].forEach(id => {
...
// ─── Export CSV ───
function exportCSV() {
  const headers = ['Name','Product ID','Barcode','Category','Quantity','Cost Price','Price (Piece)','Price (½)','Price (3/4)','Price (Half Pack)','Price (Dozen)','Price (Pack)','Price (Kilo/kg)','Price (Yard/yd)','Supplier','Date Added'];
  const rows = allProducts.map(p => [
    p.name, p.productId, p.barcode, p.category, p.quantity,
    p.costPrice, p.sellingPrice,
    p.priceHalf || '', p.priceThreequarter || '', p.priceHalfpack || '', p.priceDozens || '', p.pricePack || '',
    p.priceKilo || '', p.priceYard || '',
    p.supplier,
